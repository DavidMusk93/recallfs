...
> 对普通 `DISTINCT` 和递归 CTE loose-index-scan 做双轴实验。
...
1. **把 distinct-prefix/loose scan 做成存储层原生能力。** 需要的是
   `seek(next distinct prefix)`，不是在 SQL 层循环发起单 key 查询。
...
3. **cost model 必须同时知道 `N`、`D`、每分片 `D_s`、跨分片重复度与
   seek/RPC 成本。** 只有总行数或全局 NDV 都不足以选计划。
4. **不要无条件选择 loose scan。** 本实验在 `N=1M,D=10K` 时仍快 1.82
   倍；到 `D=100K` 时已慢 7.75 倍。应由代价模型或运行时反馈切换。
...
因此普通路径的工作接近 `O(N)`，CTE 路径更准确地说接近
`O(D log N)`，而不是严格的 `O(D)`。在 B-tree 高度稳定且内页驻留缓存时，
它实测看起来近似只随 `D` 变化。
...
索引项读完。PostgreSQL 17.10 的形状一致；其 10M case 的 `pgbench`
...
- `D`：全局 NDV；
- `S`：参与 shard 数；
- `W`：distinct key 的序列化字节数。

一个稳健的 exact distinct pipeline 是：

```text
+---------------------+
| Shard-local scan    |  N_s rows
+----------+----------+
           |
           v
+---------------------+
| Local distinct      |  D_s rows
+----------+----------+
           |
           v
+---------------------+
| Exchange by key     |  sum(D_s) rows
+----------+----------+
           |
           v
+---------------------+
| Final distinct      |  D rows
+----------+----------+
           |
           v
        result
...
去重后网络上界仍接近 `W * sum(D_s)`；最坏可到 `W * S * D`，但仍可能远
小于传原始 `W * sum(N_s)`。

本地访问方法则有两种候选：

```text
full ordered scan work ~= sum(N_s * next_cost)
loose scan work       ~= sum((D_s + 1) * seek_cost(N_s))
```

分布式 wall time 还要加入最慢 shard、exchange、spill 和 coordinator
merge。若一个 SQL 递归步骤对应一次远程 RPC，公式会多出
`D_s * rpc_latency`，足以吞掉所有收益。因此 loose scan 必须在 tablet /
range / storage node 内部循环，并按批次返回结果和 continuation token。
...
| 每 shard `D_s` | 决定局部工作与网络输出 |
| `sum(D_s)/D` | 衡量跨 shard 重复度 |
...
反馈。文章中的 `(queue_name,status)->NDV(partition_key)` 正是多列相关性
问题。

### 5.3 Distribution property
...
enqueue/dequeue/status transition 在同一事务中更新计数；`0 -> 1` 插入 active
key，`1 -> 0` 删除或标记 inactive。读取从事实表的 `O(N)` 推导变成 active
set 的 `O(D)` 扫描。
