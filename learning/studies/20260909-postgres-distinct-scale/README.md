...
- $N$：满足 `WHERE` 条件、进入去重算子之前的总行数；
- $D$：这 $N$ 行中不同 distinct key 的数量，也就是 NDV。

非空输入满足 $1\le D\le N$；空输入则有 $N=D=0$。例如，100 万条待处理
任务分属 10 个 partition，则 $N=1{,}000{,}000$、$D=10$；若分布均匀，
每个 key 平均重复 $N/D$ 次。
...
- $N$：所有参与 shard 上、谓词匹配的输入总行数；
- $D$：所有参与 shard 合并后的全局 NDV；
