...
static uint32_t bpt_crc32c_update(uint32_t crc, const unsigned char *data, size_t size) {
...
uint32_t bpt_crc32c(const void *data, size_t size) {
    ...
    return ~bpt_crc32c_update(UINT32_MAX, bytes, size);
}
...
uint32_t bpt_crc32c_page(const void *page, uint32_t page_size) {
    ...
    size_t suffix_offset = (size_t)BPT_PAGE_CHECKSUM_OFFSET + sizeof(uint32_t);

    crc = bpt_crc32c_update(crc, bytes, (size_t)BPT_PAGE_CHECKSUM_OFFSET);
    crc = bpt_crc32c_update(crc, zeros, sizeof(zeros));
    crc = bpt_crc32c_update(crc, bytes + suffix_offset, (size_t)page_size - suffix_offset);
    return ~crc;
}

bool bpt_page_checksum_valid(const void *page, uint32_t page_size) {
    const unsigned char *bytes = page;
    uint32_t stored = bpt_load_u32(bytes + BPT_PAGE_CHECKSUM_OFFSET);

    return stored == bpt_crc32c_page(page, page_size);
}

void bpt_page_checksum_store(void *page, uint32_t page_size) {
    unsigned char *bytes = page;

    bpt_store_u32(bytes + BPT_PAGE_CHECKSUM_OFFSET, 0u);
    bpt_store_u32(bytes + BPT_PAGE_CHECKSUM_OFFSET, bpt_crc32c_page(page, page_size));
}
