...
| 主推思路 | **定位不变量 + binary search**：\(i\) 是 \(A\) 的 cut（\(A\) 贡献给全局 \(L\) 几个）；在短数组上二分；边界用 \(\pm\infty\) 哨兵。 |
...
术语：\(i\) / \(j\) 是**切分点（cut）**，不是「某数组的左半个数」。

\[
L=\{A[0..i),\,B[0..j)\},\quad R=\{A[i..m),\,B[j..n)\},\quad |L|=i+j=h
\]

```text
保证 m ≤ n（在短数组上二分 cut i，保证 j∈[0,n]）
h = floor((m+n+1)/2)
...
\(j=h-i\) 必须 \(0\le j\le n\)。当 \(m\le n\) 时，对任意 \(i\in[0,m]\)：

- \(i=0\Rightarrow j=h\le n\)
- \(i=m\Rightarrow j=h-m\ge 0\)

故 \(j\) 合法区间**自动满足**；若在长数组上搜，会出现 \(j\) 越界，要额外夹逼。  
副产品：复杂度 \(O(\log\min(m,n))\)。
...
把「下中位数」固定在全局 \(L\)：奇数 median \(=L_{\max}\)；偶数再与 \(R_{\min}\) 平均。搜索过程统一。
