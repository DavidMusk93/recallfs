...
- **C 正确性工具链**：C 代码必须先使用 [FIL-C](https://fil-c.org/) 编译和测试；FIL-C 用于验证功能正确性、内存安全和未定义行为边界，不作为性能基线。将 FIL-C 及其宿主适配封装在仓库 `.tmp/fil-c/`，不得提交编译器、VM 镜像或构建产物；若当前平台不能原生运行 FIL-C，使用其官方支持的隔离 Linux 环境，不得静默降级为系统 Clang。
- **C benchmark 工具链**：性能测试必须在目标机器上切换到原生 C 编译器，并使用适合该机器和生产语义的最佳优化配置；默认至少包含 `-O3 -march=native -mtune=native -DNDEBUG`，工具链稳定支持时再启用 LTO。benchmark 前仍须通过 FIL-C 正确性验证；测量时固定 CPU/NUMA，记录 CPU 拓扑、编译器版本、完整 flags、频率策略和重复次数，并用 wall time 与硬件计数器交叉验证。不得把 FIL-C、VM、模拟器或安全插桩运行时间当作目标 CPU 的性能结论。
