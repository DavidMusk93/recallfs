...
| --- | --- | ---: | --- |
| reserve 不等于 resident | 1 GiB 匿名 `mmap`，先不触页 | reserve `4.761 us` | 建立 VMA 很便宜，不等于 RAM 已就绪 |
| first-touch 有真实成本 | 首次/再次逐 4 KiB 写同一 1 GiB | `193.459 / 3.124 ms` | 首次触页比重写慢 `61.93x` |
...
目标机环境、最终源码与二进制 digest、反汇编检查、7 轮原始统计、NUMA
绑定、page-size 覆盖、COW、cold/warm 文件路径、TLB flush 和复现命令见：

- [虚拟内存 C 实验：EPYC 目标机原生结果](virtual-memory-native-benchmark-results.md)

该附件是目标机数据的唯一详细表述。本文只保留机制结论，避免两个结果表随
后续 benchmark 修正而漂移。
...
- [目标机原生 benchmark 结果](virtual-memory-native-benchmark-results.md)
- [目标机 final6 原始向量](evidence/virtual-memory-native-20260907-final6.tsv)
